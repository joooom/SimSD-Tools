...
export default function GeneralNotes() {
  ...
  const [filters, setFilters] = useState({ committeeKey: '', delegation: '', source: '', search: '', day: '', roomId: '' });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      request(`/api/general-notes?${query}`, { signal: controller.signal })
        .then(data => { if (!controller.signal.aborted) { setNotes(data.notes); setSessions(data.sessions || []); } })
  ...
  const save = async event => {
    ...
    try {
      await request(`/api/general-notes${editing ? `/${encodeURIComponent(editing.id)}` : ''}`, {
      ...
      setEditing(null); setForm(current => editing?.source === 'session' ? emptyForm() : { ...current, text: '', ratings: {} });
      setRefresh(value => value + 1);
    } catch (err) { setFormError(err.message); }
    finally { setSaving(false); }
  };
  const edit = note => {
    setEditing({ id: note.id, version: note.version, source: note.source });
  ...
  const remove = async note => {
    ...
    setDeleting(note.id); setError(''); setMessage('');
    try {
      await request(`/api/general-notes/${encodeURIComponent(note.id)}`, { method: 'DELETE', body: JSON.stringify({ version: note.version }) });
      if (editing?.id === note.id) reset();
      setMessage('Nota excluída.'); setRefresh(value => value + 1);
  ...
  return <section className="general-notes" aria-label="Notas gerais">
    ...
    <div className="general-notes-grid">
      <section className="general-note-form" ref={formRef}>
        <h3>{editing ? 'Editar nota' : 'Nova nota fora de sessão'}</h3>
        <p>{editing?.source === 'session' ? 'Edite o texto e os critérios. O vínculo com a sessão, delegação e discurso será preservado.' : 'Selecione o comitê e a delegação. Você pode escrever uma observação, avaliar critérios ou fazer ambos.'}</p>
        <form onSubmit={save}>
          <fieldset disabled={saving || Boolean(deleting)}>
            {editing?.source === 'session' ? <p>{COMMITTEE_NAMES[form.committeeKey]} · {form.participant || 'Nota geral da sessão'} · {noteTypes[form.type] || form.type}</p> : <>
      ...
      <section className="general-notes-history" aria-label="Consulta centralizada">
        ...
        <div className="general-notes-filters">
          <label>Filtrar comitê<select value={filters.committeeKey} onChange={event => setFilters(current => ({ ...current, committeeKey: event.target.value, delegation: '', roomId: '' }))}><option value="">Todos os comitês</option>{Object.entries(COMMITTEE_NAMES).map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></label>
          <label>Filtrar sessão<select value={filters.roomId} onChange={event => setFilters(current => ({ ...current, roomId: event.target.value, ...(event.target.value ? { source: 'session' } : {}) }))}><option value="">Todas as sessões</option>{sessions.filter(session => !filters.committeeKey || session.committeeKey === filters.committeeKey).map(session => <option key={session.id} value={session.id}>{COMMITTEE_NAMES[session.committeeKey]} · {session.roomName}{session.name ? ` · ${session.name}` : ''} · {session.code} ({session.status === 'closed' ? 'encerrada' : 'aberta'})</option>)}</select></label>
          ...
          <label>Origem<select value={filters.source} onChange={event => setFilters(current => ({ ...current, source: event.target.value, ...(event.target.value === 'general' ? { roomId: '' } : {}) }))}><option value="">Notas gerais e sessões</option><option value="general">Fora de sessão</option><option value="session">Notas das sessões</option></select></label>
          <label>Dia das notas e acontecimentos (Brasília)<input type="date" value={filters.day} onChange={event => setFilters(current => ({ ...current, day: event.target.value }))} /></label>
        ...
        <p className="notes-count">{loading ? 'Carregando notas…' : `${notes.length} nota(s) encontrada(s). A exportação inclui somente notas e acontecimentos que correspondem aos filtros selecionados. O dia considera o horário de Brasília; acontecimentos sem data completa ficam de fora desse filtro.`}</p>
        ...
        <div aria-busy={loading} className="central-notes-list">
          ...
          {notes.map(note => <article key={note.id} className="central-note">
            <div className="central-note-header"><span className={`note-origin ${note.source}`}>{note.source === 'general' ? 'Fora de sessão' : 'Sessão'}</span><span>{noteTypes[note.type] || note.type}</span>{note.canEdit && <div className="central-note-actions"><button disabled={saving || Boolean(deleting)} onClick={() => edit(note)}>Editar</button><button disabled={saving || Boolean(deleting)} onClick={() => remove(note)}>{deleting === note.id ? 'Excluindo…' : 'Excluir'}</button></div>}</div>
